...
export default function EventCard({ event, priority = false }: EventCardProps) {
  ...
  return (
    <Card variant="bordered" className="hover:shadow-lg transition-shadow h-full flex flex-col">
      ...
      <div className="p-4 flex flex-col grow">
        <div className="mb-2 h-6 shrink-0">
        ...
        <Link href={`/events/${event.slug}`}>
          <h3 className="text-lg font-semibold text-gray-900 mb-2 hover:text-blue-600 transition-colors line-clamp-2 h-14 shrink-0">
            {event.title}
          </h3>
        </Link>

        <p className="text-gray-600 text-sm mb-3 line-clamp-2 h-10 shrink-0">
          {event.description}
        </p>

        <div className="space-y-2 text-sm text-gray-500 mb-4 shrink-0">
        ...
        <Link href={`/events/${event.slug}`} className="block mt-4 shrink-0">
